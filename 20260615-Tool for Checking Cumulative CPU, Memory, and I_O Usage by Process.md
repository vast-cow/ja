...
description: "概要 Linux 上で動作するシンプルなプロセス監視ツールです。 通常の top は瞬時値を確認できますが、本ツールは CPU時間や読み書き の積算を表示できます。 目的 このツールの主な目的は、プロセスごとの負荷を簡単に確認することです。 CPU やメモリだけでなく、読み込み量、書き込み量、読み書…"
---
