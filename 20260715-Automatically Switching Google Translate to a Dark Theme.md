...
description: "Google翻訳を長時間使うと、明るい画面がまぶしく感じることがあります。そこで便利なのが、パソコンやブラウザの表示設定に合わせて、Google翻訳を自動的にダークテーマへ切り替えるユーザースクリプトです。 このスクリプトを導入すると、端末がダークモードのときだけGoogle翻訳の背景や入力欄、翻訳…"
---
