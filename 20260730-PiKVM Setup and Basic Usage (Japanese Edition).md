...
description: "PiKVMは、管理対象のコンピューターにOSが起動していない状態でも、映像の確認、キーボード・マウス操作、仮想メディアからの起動などを行えるリモートKVMです。 本記事では、SDカードへのイメージ書き込み後から、初期設定、仮想メディアの配置、Tailscaleによるリモートアクセス、管理対象マシンの…"
---
