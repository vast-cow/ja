...
description: TCP接続の試行を維持しながら、常時起動のスタンバイノードと一時停止中のターゲットマシン間で、固定のサービスIP/MACアドレスを確実に引き継げるかどうかを検証する事前実験。Linuxのネットワークネームスペース、macvlan、および制御されたパケットの破棄を利用して、TCPの再試行動作、ARP/FDBの収束、およびオンデマンドのサスペンド＋Wake-on-LANアクセスシステムを構築するための要件を検証する。
---
